...
const useExcelImport = (setUploadOrders, setManyOrders) => {
    ...
    reader.onload = (e) => {
      ...
      const parsedOrders = sheetData
        .map((row, rowIndex) => {
          ...
          if (
            String(newRow["productid"]).includes(",") &&
            String(newRow["productid"]).includes(".")
          ) {
            errors.push(
              `Row ${
                rowIndex + 2
              }: Product ID must be separated by one of , or . only`
            );
            rowHasError = true;
          }
          if (
            String(newRow["quantity"]).includes(",") &&
            String(newRow["quantity"]).includes(".")
          ) {
            errors.push(
              `Row ${
                rowIndex + 2
              }: quantity must be separated by one of , or . only`
            );
            rowHasError = true;
          }
          ...
          if (String(newRow["productid"]).includes(",")) {
            productsArray = String(newRow["productid"])
              .split(",")
              .map((e) => e.trim());
          } else if (String(newRow["productid"]).includes(".")) {
            productsArray = String(newRow["productid"])
              .split(".")
              .map((e) => e.trim());
          } else {
            productsArray.push(String(newRow["productid"]).trim());
          }

          let quantitiesArray = [];
          if (String(newRow["quantity"]).includes(",")) {
            quantitiesArray = String(newRow["quantity"])
              .split(",")
              .map((e) => e.trim());
          } else if (String(newRow["quantity"]).includes(".")) {
            quantitiesArray = String(newRow["quantity"])
              .split(".")
              .map((e) => e.trim());
          } else {
            quantitiesArray.push(String(newRow["quantity"]).trim());
          }
  ...
  const handleConfirmExcelImport = useCallback(async () => {
    ...
    try {
      ...
      await setManyOrders(dataToSend, dispatch);
  ...
  }, [orders, setUploadOrders, setManyOrders, dispatch]);
